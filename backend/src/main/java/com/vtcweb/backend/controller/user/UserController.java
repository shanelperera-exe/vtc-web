...
public class UserController {
    ...
    public UserController(UserService userService, OrderService orderService, UserAddressService userAddressService) { this.userService = userService; this.orderService = orderService; this.userAddressService = userAddressService; }

    // Current user endpoints
    @GetMapping("/users/me")
    public ResponseEntity<UserDto> current() { return ResponseEntity.ok(userService.getCurrent()); }

    @PutMapping("/users/me")
    public ResponseEntity<UserDto> updateCurrent(@Valid @RequestBody UserUpdateRequest req) { return ResponseEntity.ok(userService.updateCurrent(req)); }

    @PostMapping("/users/me/change-password")
    public ResponseEntity<Void> changePassword(@Valid @RequestBody ChangePasswordRequest req) { userService.changePasswordCurrent(req); return ResponseEntity.noContent().build(); }

    // Admin / Manager endpoints (MANAGER can do everything; ADMIN limited to customer accounts enforced in service layer)
    ...
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<UserDto> get(@PathVariable Long id) { return ResponseEntity.ok(userService.getById(id)); }
    ...
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<UserDto> create(@Valid @RequestBody UserCreateRequest req) { return ResponseEntity.status(HttpStatus.CREATED).body(userService.create(req)); }

    @PutMapping("/admin/users/{id}")
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<UserDto> update(@PathVariable Long id, @Valid @RequestBody UserUpdateRequest req) { return ResponseEntity.ok(userService.update(id, req)); }
    ...
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    public ResponseEntity<UserDto> updateStatus(@PathVariable Long id, @Valid @RequestBody UserStatusUpdateRequest req) {
        return ResponseEntity.ok(userService.updateStatus(id, Boolean.TRUE.equals(req.getEnabled())));
    }

    // Manager-only endpoint to change roles (MANAGER can assign roles including ADMIN); service prevents unlawful downgrades if needed
