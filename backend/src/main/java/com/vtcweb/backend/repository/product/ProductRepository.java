...
public interface ProductRepository extends JpaRepository<Product, Long> { // JpaRepository provides CRUD operations for Product entity with Long as ID type
    ...
    Page<Product> findByCategory_IdAndStatus(Long categoryId, com.vtcweb.backend.model.entity.product.ProductStatus status, Pageable pageable);
    ...
    Page<Product> findByNameContainingIgnoreCaseAndStatus(String name, com.vtcweb.backend.model.entity.product.ProductStatus status, Pageable pageable);
    ...
    Page<Product> findInStockByStatus(@org.springframework.data.repository.query.Param("status") com.vtcweb.backend.model.entity.product.ProductStatus status, Pageable pageable);
    ...
    Page<Product> findInStockByCategory(@org.springframework.data.repository.query.Param("categoryId") Long categoryId, Pageable pageable);
    ...
    Page<Product> findInStockByCategoryAndStatus(@org.springframework.data.repository.query.Param("categoryId") Long categoryId, @org.springframework.data.repository.query.Param("status") com.vtcweb.backend.model.entity.product.ProductStatus status, Pageable pageable);

    // Out-of-stock = no variations with stock > 0 (including products with zero variations)
    @org.springframework.data.jpa.repository.Query("select p from Product p left join p.variations v group by p having coalesce(sum(case when v.stock > 0 then 1 else 0 end),0) = 0")
    Page<Product> findOutOfStock(Pageable pageable);

    @org.springframework.data.jpa.repository.Query("select p from Product p left join p.variations v where p.status = :status group by p having coalesce(sum(case when v.stock > 0 then 1 else 0 end),0) = 0")
    Page<Product> findOutOfStockByStatus(@org.springframework.data.repository.query.Param("status") com.vtcweb.backend.model.entity.product.ProductStatus status, Pageable pageable);

    @org.springframework.data.jpa.repository.Query("select p from Product p left join p.variations v where p.category.id = :categoryId group by p having coalesce(sum(case when v.stock > 0 then 1 else 0 end),0) = 0")
    Page<Product> findOutOfStockByCategory(@org.springframework.data.repository.query.Param("categoryId") Long categoryId, Pageable pageable);

    @org.springframework.data.jpa.repository.Query("select p from Product p left join p.variations v where p.category.id = :categoryId and p.status = :status group by p having coalesce(sum(case when v.stock > 0 then 1 else 0 end),0) = 0")
    Page<Product> findOutOfStockByCategoryAndStatus(@org.springframework.data.repository.query.Param("categoryId") Long categoryId, @org.springframework.data.repository.query.Param("status") com.vtcweb.backend.model.entity.product.ProductStatus status, Pageable pageable);
}
