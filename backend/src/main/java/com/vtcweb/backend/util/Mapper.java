...
/**
 * Simple manual mapper to keep controllers/services decoupled from JPA entities
 * at the API layer.
 */
public final class Mapper {
    ...
    /**
     * Detailed mapping: expects relations (images, variations and variation
    ...
    public static Product fromCreateRequest(CreateProductRequest req) {
        ...
        if (req.getStatus() != null) {
            try {
                product.setStatus(com.vtcweb.backend.model.entity.product.ProductStatus
                        .valueOf(req.getStatus().trim().toUpperCase()));
    ...
    public static void applyUpdates(Product target, UpdateProductRequest updates) {
        ...
        if (updates.getStatus() != null) {
            try {
                target.setStatus(com.vtcweb.backend.model.entity.product.ProductStatus
                        .valueOf(updates.getStatus().trim().toUpperCase()));
            } catch (IllegalArgumentException ignored) {
                
            }
        }
        if (updates.getHighlights() != null) {
            // Replace entire list (client sends authoritative ordering); filter blank
            // entries
