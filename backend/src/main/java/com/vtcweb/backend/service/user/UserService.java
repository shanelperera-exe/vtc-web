...
public class UserService {
    ...
    //Getting relevant users by their roles
    public List<User> getUsersByRole(String role) {
        return userRepository.findByRole(role);
    }

    //Checks whether their is a user with the given email
    public boolean userExists(String email) {
        return userRepository.existsByEmail(email);
    }

    //Changes the role of the user given by the id
    public User changeUserRole(Long id, String newRole) {
        User user = getUserById(id);
        user.setRole(com.vtcweb.backend.model.entity.user.UserRole.valueOf(newRole));
        return userRepository.save(user);
    }
}