...
@EqualsAndHashCode(onlyExplicitlyIncluded = true) // Lombok: Generates equals() and hashCode() using only fields marked
                                                  // with @EqualsAndHashCode.Include
@Builder // Generates a builder pattern, useful for creating objects in a readable way
public class Category {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY) // lets the database auto-generate IDs using the identity column
                                                        // strategy (Auto Incrementing ID)
    @ToString.Include
    ...
    /**
     * Activity status for the category. ACTIVE categories are visible/selectable;
     * INACTIVE are hidden/disabled.
     */
     ...
     * NOTE: Avoid cascading remove from category -> product to prevent accidental
     * data loss.
     */
