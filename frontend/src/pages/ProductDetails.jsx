...
function normalizeProduct(p) {
  ...
  const primary =
    p.primaryImageUrl || p.mainImageUrl || p.primaryImage || p.image || "";
  ...
  const colorsFromVars = Array.from(
    new Set(
      variations
        .map((v) => (v.attributes?.Color || v.attributes?.color || "").trim())
        .filter(Boolean)
    )
  );
  const sizesFromVars = Array.from(
    new Set(
      variations
        .map((v) => (v.attributes?.Size || v.attributes?.size || "").trim())
        .filter(Boolean)
    )
  );
  ...
  if (variations.length) {
    ...
    for (const v of variations) {
      const col =
        (v.attributes?.Color || v.attributes?.color || "").trim() || "Default";
      const size =
        (v.attributes?.Size || v.attributes?.size || "").trim() || "Default";
  ...
  return {
    ...
    description:
      shortDesc ||
      (longDesc.length > 160 ? longDesc.slice(0, 160).trim() + "…" : longDesc),
    ...
    availableColors: (p.availableColors || p.colors || []).length
      ? p.availableColors || p.colors
      : colorsFromVars,
    availableSizes: (p.availableSizes || p.sizes || []).length
      ? p.availableSizes || p.sizes
      : sizesFromVars,
    // prefer availability built from variations, else pass-through legacy
    availability:
      availability || p.availability || p.stockStatus || p.stock || null,
...
export default function ProductDetails() {
      ...
      try {
        ...
        if (sku && !isNumeric(sku)) {
          ...
          try {
            ...
            const fromSku =
              page?.content && page.content[0] ? page.content[0] : null;
            if (fromSku) {
              // After shallow fetch by SKU, get full details by id
              try {
                p = await getProductDetails(fromSku.id);
              } catch (e) {
        ...
        if (!p && sku) {
          ...
          if (idNum != null) {
            try {
              p = await getProductDetails(idNum);
            } catch (_E2) {
              p = await getProduct(idNum);
            }
          }
        }
        ...
        if (active) {
          ...
          try {
            if (p?.id) {
              ...
              if (active)
                setRawProduct((cur) => ({ ...(p || cur), reviews: rev }));
            }
          } catch (_E3) {
  ...
  const sortedSizes = useMemo(() => {
    ...
    if (!vars.length) {
      return Array.isArray(product?.availableSizes)
    ...
    for (const v of vars) {
      ...
      const price = Number(
        v.price ?? v.basePrice ?? product.price ?? Number.POSITIVE_INFINITY
      );
      ...
      if (selColor && color === selColor) {
        cur.colorPrice = Math.min(
          cur.colorPrice ?? Number.POSITIVE_INFINITY,
          price
        );
      }
    ...
    const arr = Array.from(sizeMap.entries()).map(([size, info]) => {
      const effective =
        selColor && info.colorPrice !== undefined && isFinite(info.colorPrice)
          ? info.colorPrice
          : info.anyMin ?? Number.POSITIVE_INFINITY;
      return { size, price: effective };
    });
    arr.sort((a, b) =>
      a.price === b.price ? a.size.localeCompare(b.size) : a.price - b.price
    );
  ...
  const numReviews = useMemo(() => {
    ...
    const backendCount =
      typeof product?.numOfReviews === "number" ? product.numOfReviews : null;
  ...
  const rating = useMemo(() => {
    ...
    if (Array.isArray(revs) && revs.length) {
      const sum = revs.reduce(
        (s, r) => s + (Number(r.rating) || Number(r.stars) || 0),
        0
      );
  ...
  const allImages = useMemo(() => {
    ...
    const arr = [product.image, ...(product.secondaryImages || [])].filter(
      Boolean
    );
    const uniq = [];
    const seen = new Set();
  ...
  const selectedImg = allImages[selectedImgIdx] || allImages[0];
  const handlePrevImg = () => {
    setSelectedImgIdx(
  ...
  const handleNextImg = () => {
    setSelectedImgIdx((prev) => (prev + 1) % allImages.length);
  };
  ...
  const unitsLeft = useMemo(() => {
    ...
    if (Array.isArray(product?.variations) && product.variations.length) {
      ...
      const filtered = product.variations.filter((v) => {
        ...
        const vs = (a.Size || a.size || "").toLowerCase();
        return (
          (selColor ? vc === selColor : true) &&
          (selSize ? vs === selSize : true)
        );
      });
  ...
  if (!product) {
    return (
      <>
        <Navbar />
        <div className="p-12 text-center text-xl text-red-500">
          Product not found.
        </div>
      </>
    );
      ...
      <div className="bg-white">
        <div className="pt-6">
          <nav aria-label="Breadcrumb">
            <ol
              role="list"
              className="flex max-w-2xl items-center space-x-2 pl-6 sm:pl-10 lg:max-w-7xl lg:pl-16"
            >
              ...
              <li className="text-sm">
                <a
                  href="#"
                  aria-current="page"
                  className="font-large text-gray-500 hover:text-gray-600"
                >
                  {product.name}
                </a>
              </li>
          ...
          <div className="mt-6 max-w-5xl pl-6 sm:pl-10 lg:max-w-[800px] lg:pl-16">
            <div className="flex flex-col gap-8 lg:flex-row">
              ...
              <div className="flex-1 flex justify-start items-start">
                ...
                <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 lg:sticky lg:top-24 self-start">
                  <div className="flex lg:flex-col flex-row gap-2 justify-center items-center lg:items-start lg:justify-start lg:max-h-[550px] pr-2 overflow-x-auto lg:overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 order-2 lg:order-0">
                    {allImages.map((img, idx) => (
                      <button
                        type="button"
                        key={img + idx}
                        className={`bg-gray-100 rounded-lg flex items-center justify-center w-12 h-12 lg:w-[110px] lg:h-[110px] cursor-pointer border transition focus:outline-none overflow-hidden ${
                          selectedImgIdx === idx
                  ...
                  <div className="bg-gray-100 flex items-center justify-center w-[358px] h-[358px] mx-auto sm:w-[420px] sm:h-[420px] lg:w-[550px] lg:h-[550px] min-w-0 lg:min-w-[300px] min-h-[200px] lg:min-h-[300px] rounded-2xl overflow-hidden order-1 lg:order-0 relative">
                    {/* Carousel left button */}
                    {allImages.length > 1 && (
                      <button
                        type="button"
                        className="absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-lg p-3 shadow-md"
                    ...
                    {allImages.length > 1 && (
                      <button
                        type="button"
                        className="absolute right-2 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-lg p-3 shadow-md"
              ...
              <div className="flex-[1.2] flex flex-col gap-3 lg:ml-6 w-full">
                <h1 className="text-3xl lg:text-6xl font-medium tracking-tight">
                  {product.name}
                </h1>
                {/* SKU and stock pill for mobile only */}
                <div className="flex flex-col gap-1 sm:hidden mb-1">
                  <div className="flex items-center gap-3">
                    {product.sku && (
                      <div className="mr-2 text-md text-gray-500">
                        SKU:{" "}
                ...
                {/* Star review and num reviews, always visible */}
                <div className="flex items-center justify-start gap-6">
                  <StarReview
                    ...
                    showCount={true}
                  />
                  {/* SKU and stock pill for desktop only */}
                  <div className="text-md text-gray-500 items-center gap-3 hidden sm:flex">
                    {product.sku && (
                      <div className="mr-2">
                        SKU:{" "}
                        <span className="font-medium text-gray-900">
                  ...
                  <div className="mt-2">
                            ...
                            <div className="flex items-center gap-x-2 w-44 sm:w-auto">
                              {colorOptions.map((color) => {
                                ...
                                return (
                                  <div
                                    key={color}
                                    className={`relative flex outline -outline-offset-1 outline-black/10 rounded-md ${
                    ...
                    <div className="mb-4">
                      <div className="flex items-center gap-2 text-sm">
                        <FiPackage
                          className={`${
                            unitsLeft > 0 ? "text-green-600" : "text-red-600"
                        ...
                        {unitsLeft != null ? (
                          unitsLeft > 0 ? (
                            <span className="text-green-700 font-semibold">
                              Hurry! Only {unitsLeft}{" "}
                              {unitsLeft === 1 ? "item" : "items"} left in
                              stock.
                            </span>
                          ) : (
                    ...
                    <div className="flex flex-col gap-3 mb-6">
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full">
                        {/* Row for mobile: Quantity + Wishlist */}
                        ...
                        <div className="hidden sm:flex flex-row items-center gap-4 w-full">
                          ...
                          <AddToCartBtn
                            product={{
                              ...
                              color: selectedColor,
                              size: selectedSize,
                              stock: unitsLeft ?? effectiveStock,
                            }}
                            quantity={qty}
                          />
                          <AddToWishlistBtn
                        ...
                        {/* Add to cart button for mobile, below row */}
          ...
          <div className="w-full mt-8 sm:px-0 px-0 text-left -ml-5 sm:ml-0">
            <div className="sm:text-base text-sm w-full p-0 m-0">
              <ProductTabs
                product={
                  product
                    ? {
                ...
                // Responsive review section tweaks for mobile
                reviewCardClassName="sm:p-6 p-3 sm:mb-4 mb-2 sm:rounded-xl rounded-lg sm:text-base text-xs sm:gap-4 gap-2 w-full break-words text-left items-start p-0 m-0 -ml-3 sm:ml-0"
                reviewAvatarClassName="sm:w-12 sm:h-12 w-8 h-8"
                reviewTextClassName="sm:text-base text-xs break-words text-left"
                reviewNameClassName="sm:text-sm text-xs text-left"
                reviewDateClassName="sm:text-xs text-[10px] text-left"
                reviewStarClassName="sm:w-5 sm:h-5 w-4 h-4"
                reviewSummaryRowClassName="flex-col sm:flex-row flex items-start sm:items-center gap-0 sm:gap-2"
              />
            </div>
          </div>
