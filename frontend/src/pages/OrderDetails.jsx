...
const OrderDetails = () => {
        ...
        <main className="mx-auto max-w-3xl py-16 px-8">
          <div className="bg-white shadow-sm border border-black/10 rounded-xl p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              You must be signed in to view order details. Please sign in with
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      ...
      for (const it of effectiveOrder.items) {
        ...
        if (!pid || Object.prototype.hasOwnProperty.call(productCache, pid))
          continue;
        try {
      ...
      <>
        ...
        <main className="mx-auto max-w-7xl py-16 px-8">
          <div className="p-6 text-sm text-red-600">
            You must be logged in to view order details. Please{" "}
    ...
    return orderItems.map((item) => {
      ...
      const pickImage = (prod, ordItem) => {
        ...
        if (Array.isArray(prod.images)) {
          const first = prod.images.find(
            (i) => i && (i.url || i.path || i.src)
          );
      ...
      if (!image && process.env.NODE_ENV !== "production") {
        ...
        console.debug(
          "OrderDetails: no image found for product",
          item.productId,
          item.variationId,
          base
        );
      ...
      return {
        ...
        category:
          base?.category?.name || base?.category || item.categoryName || "",
  ...
  const statusLabel =
    orderProgress === -1
      ? "Cancelled"
      : statusSteps[orderProgress] ?? effectiveOrder.status;
  const statusDetail = (() => {
    const times = effectiveOrder?.statusTimes || {};
    if (orderProgress === -1) {
      const dt = times.cancelled;
      if (dt?.date)
        return `Order cancelled on ${dt.date}${dt.time ? ` ${dt.time}` : ""}`;
      return "Order cancelled";
    }
    if (orderProgress === 0) {
      const dt = times.placed;
      if (dt?.date)
        return `Order placed on ${dt.date}${dt.time ? ` ${dt.time}` : ""}`;
      return `Order placed on ${effectiveOrder.placed}${
        effectiveOrder.placedTime ? ` ${effectiveOrder.placedTime}` : ""
      }`;
    }
                ...
                <div key={safeKey} className="col-span-12 md:col-span-6">
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        If you’ve used this product, <br />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700">
                  {effectiveOrder.deliveryMethod ?? "Standard delivery"}
                </p>
              </div>
            </div>
          </div>
