...
const _reviewSummaryCache =
  typeof window !== "undefined"
    ? (window.__vtcReviewSummaryCache ||= new Map())
    : new Map();
...
const ProductCard = ({
  ...
  const [localCount, setLocalCount] = useState(
    typeof numOfReviews === "number" ? numOfReviews : 0
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (cached) {
        if (active) {
          setLocalRating(cached.rating);
          setLocalCount(cached.count);
        }
        return;
      }
      try {
  ...
  const productRating = useMemo(
    () =>
      Math.max(
        0,
        Math.min(5, typeof localRating === "number" ? localRating : 0)
      ),
    [localRating]
  );
      ...
      <div className="card" style={{ cursor: "pointer" }}>
        ...
        <div className="absolute right-3 top-3 z-10">
          <ProductCardWishlistBtn
            ...
            isWishlisted={
              typeof isWishlisted === "boolean" ? isWishlisted : undefined
            }
        ...
        <div>
          ...
          <div>
            <div
              className={`card-info ${titleTwoLines ? "title-multiline" : ""}`}
            >
              <p
                ref={titleRef}
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex flex-col items-end space-y-1">
                  ...
                  <div className="text-xs font-medium text-[#1d794c]">
                    {localCount ?? 0}{" "}
                    {Number(localCount) === 1 ? "review" : "reviews"}
                  </div>
                </div>
              </div>
            </div>
