...
const OrderStatusBar = ({
  ...
  const progressRatio =
    progress >= maxStep ? 1 : (progress * 2 + 1) / (maxStep * 2);
  ...
  const renderBadge = () => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-semibold ${badgeClasses}`}
      >
    ...
    <div className="w-full pb-3">
      ...
      {statusLabel || statusDetail ? (
        <section aria-labelledby="order-status-heading" className="mb-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
              <h2
                ...
                className="text-3xl font-semibold inline-flex items-center gap-2"
              >
                <FiCheckCircle
            ...
            {headerActions ? <div className="ml-4">{headerActions}</div> : null}
          </div>
          {statusDetail && (
            <div className="mb-3">
          ...
          {(orderStatus || "").toLowerCase().includes("cancel") &&
            statusTimes &&
            statusTimes.cancelled && (
              <div className="mb-2">
                <p className="text-sm text-red-700 font-medium">
                  Cancelled on {statusTimes.cancelled.date}{" "}
                  {statusTimes.cancelled.time}
                </p>
              </div>
            )}
        </section>
      ...
      <div className="block sm:hidden w-full">
        <div className="flex flex-row items-stretch gap-0 py-4 px-2 relative">
          {/* Vertical track */}
          <div
            className="absolute left-5 top-6 bottom-6 w-2 bg-gray-200 rounded-md"
            style={{ zIndex: 0 }}
          />
          {/* Progress fill */}
          <div
            ...
            style={{
              ...
              background: (orderStatus || "").toLowerCase().includes("cancel")
                ? "#fdecea"
                : "#0bd964",
            }}
          />
          ...
          <div className="flex flex-col gap-2 z-10">
            {statusIcons.map((Icon, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex items-center justify-center border-3 shadow-sm ${border} rounded-md`}
      ...
      <div className="hidden sm:block w-full">
        <div
          className="relative w-full px-10 pt-10 pb-4"
          style={{
            "--track-width": `calc(100% - ${ICON_SIZE}px)`,
            "--icon-half": `${ICON_HALF}px`,
          }}
        >
          ...
          {(orderStatus || "").toLowerCase().includes("cancel") ? (
            ...
            <div
              ...
              style={{
                left: ICON_HALF,
                width: `calc(var(--track-width) * ${progressRatio})`,
              }}
            />
          )}
          ...
          {statusIcons.map((Icon, idx) => {
            ...
            return (
              <div
                ...
                style={{
                  top: 0,
                  left: `calc(var(--icon-half) + var(--track-width) * ${leftPct})`,
