...
const ProductInOrder = ({ product, address, shippingUpdates, showStatus = true, children }) => {
  return (
  <div className="bg-white shadow-sm p-6 flex flex-col gap-4 mb-4 overflow-hidden border-3 border-black">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:gap-8 gap-6 w-full">
          <div className="flex-shrink-0">
            {product.image ? (
              <img
                src={product.image}
                alt={product.name}
                className="w-36 h-36 object-cover border-2 bg-gray-200"
              />
            ) : (
              <div className="w-36 h-36 flex items-center justify-center border-2 bg-gray-100 text-gray-400">
          ...
          <div className="flex-1 flex flex-col justify-center">
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              <span title={product.category} className="inline-block text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-700 truncate whitespace-nowrap max-w-full">{product.category}</span>
            </p>
