...
const PlaceOrder = ({ onClick, disabled = false, children = 'Place Order' }) => {
  ...
  const handleClick = (e) => {
    ...
    try {
      const res = onClick?.(e);
      // optimistically mark placed (works for sync or promise-returning handlers)
      setPlaced(true);
      return res;
    } catch (err) {
      console.error(err);
  ...
  return (
    <StyledWrapper>
      ...
      >
        <span className="btn-text">{placed ? 'Order placed' : children}</span>
        <div className="icon-container" aria-hidden>
...
const StyledWrapper = styled.div`
  .pay-btn {
    position: relative;
    padding: 10px 20px; /* match px-5 py-2.5 */
    font-size: 0.875rem; /* text-sm */
    background: #059669; /* emerald tone to match primary action */
    color: white;
    border: none;
